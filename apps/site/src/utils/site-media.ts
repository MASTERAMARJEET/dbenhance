...
function instagramSrc(shortcode: string): string {
	return siteImages.instagram[shortcode as keyof typeof siteImages.instagram] ?? "";
}
...
/** Stacked images for the about page sidebar. */
export function getPageImageStack(): ImageAsset[] {
	return siteImages.pages.aboutStack.map((shortcode, index) => ({
		src: instagramSrc(shortcode),
		alt:
			index === 0
				? "Salon work at DB Enhance"
				: "Hair styling at DB Enhance",
	}));
}
